print("gy，请打人")
function attackHook(){var ctx=com.mojang.minecraftpe.MainActivity.currentMainActivity.get()
var btn=null
var n=null
...
var allenttype=[["蜘蛛",35],["骷髅",34],["苦力怕",33],["僵尸",32],["鸡",10],["牛",11],["猪",12],["羊",13],["狼",14],["蘑菇牛",16],["蠹虫",39],["末影人",38],["史莱姆",37],["僵尸猪人",36],["矿车",84],["村民",15],["掉落物品",64],["TNT",65],["下落的方块",66],["弓箭",80],["雪球",81],["鸡蛋",82],["壁画",83]]

var entitylist=[]
var entityid=[]

var entstop=[]
var entityretype=[true]
var entityrelist=["所有实体"]
for(var m in allenttype){
entityretype.push(allenttype[m][1])
entityrelist.push(allenttype[m][0])
if(m<16)
{
entitylist.push(allenttype[m][0])
entityid.push(allenttype[m][1])
}
}
...
if(m==63&&Player.getCarriedItem()==345){
preventDefault();
...
ctx.getWindow().addFlags(android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
				...
				if(openWindow==null){openWindow = new android.widget.PopupWindow(layout, dip2px(ctx, 35), dip2px(ctx, 35));
...
print2("已恢复上一次功能")
			...
			if(guis==null){	var openWindow = new android.widget.PopupWindow(layout, dip2px(ctx, 35), dip2px(ctx, 35));
...
Entity.setPosition(getplayerent(),x+xv,y,z+zv);
}}))
layout.addView(button)   
...
if(che==0){settilelist=[];shentoutick=[];settilenum=0}
}
...
{s3=[x,y,z];print2("设置点三");}
else if(s4==false){s4=[x,y,z];print2("连接");sdqm(s1,s2,s3,s4,i,id);cleans();geotype=0}}
...
case 206:tree6(x,y,z,b,bd);break; 
...
if(shentoutick[x+","+y+","+z])return;
shentoutick[x+","+y+","+z]=true
...
try{if(shentoutick[x+","+y+","+z])return;
shentoutick[x+","+y+","+z]=true}catch(e){print(e)}
...
function tree6(x,y,z,b,bd){
var pi=Math.PI/180////
var lx=function(s1,s2,i,id){
var x=s1[0],y=s1[1],z=s1[2],x2=s2[0],y2=s2[1],z2=s2[2]
var l=Math.max(Math.abs(x-x2),Math.abs(y-y2),Math.abs(z-z2))
for(var m=0;m<=l;m++){
var w=-1
if(i==17)w=1
var a=Math.round(x+m/l*(x2-x)),b=Math.round(y+m/l*(y2-y)),c=Math.round(z+m/l*(z2-z))///
settile(a,b+1,c,i+w,id)
settile(a,b-1,c,i+w,id)
settile(a+1,b,c,i+w,id)
settile(a-1,b,c,i+w,id)
settile(a,b,c-1,i+w,id)
settile(a,b,c+1,i+w,id)
settile(a,b-2,c,i+w,id)
settile(a,b,c,i,id)
}}
if(b==17){
var l=27+Math.random()*5
var o=Math.random()*3+7
for(var m=0;m<o;m++){
for(var k=0;k<1;k+=0.2*m/o+0.25){
var is=360*pi*k
var yaw=Math.random()*7200*pi
var l2=(1-m/o)*4+1
var s1=[x,y+l/o*m+5+4,z]
var a=Math.cos(yaw+is)*l2
var b=Math.sin(-Math.random()*60*pi)*l2*0.23
var c=Math.sin(yaw+is)*l2
var s2=[x+a,y+l/o*m+5+b,z+c]///
lx(s1,s2,17,12+bd%4)
}}lianxian([x,y,z],[x,y+l,z],17,bd);}
if(b==162){
var l=27+Math.random()*5
var o=Math.random()*3+7
for(var m=0;m<o;m++){
...
var c=Math.sin(yaw+is)*l2
var s2=[x+a,y+l/o*m+5+b,z+c]///
lx(s1,s2,162,12+bd%4)
}}lianxian([x,y,z],[x,y+l,z],162,bd);}
}
...
var isgetchunk=false
var iskq=false;
function getchunk(c,pxyz,px,pz,cg,cb)
{if(!isgetchunk){isgetchunk=true
var xcs=new java.lang.Thread(new java.lang.Runnable({run:function(){
//try{
c[pxyz]=null
var gx=getPlayerX()%16
var gz=getPlayerZ()%16
var gxyz//=""+gx+100+gz
//if(c[pxyz]==undefined){
//if(getTile(16*px,0,16*pz)>0){
var id,bd
//print("num"+pxyz)
var sp=[]
for(var a=0;a<16;a++){
for(var b=0;b<16;b++){
var at=1
for(var h=64;h>0;h-=1){
id=getTile(16*px+a,h,16*pz+b)
if(id>0){
bd=Level.getData(16*px+a,h,16*pz+b);break
}}
gxyz="s"+a+100+b
if(cg[id]!=undefined){
if(cg[id][bd]!=undefined){sp[gxyz]=cg[id][bd][1]}else{sp[gxyz]=cb[id][at]}
}else{sp[gxyz]=cb[id][at]}
...
var c=[]
var numb=[]
   for(var a=0; a<=fw ;a+=16)
   {
     for(var b=0; b<=fw ;b+=16)
     {
      if((fw/2)*(fw/2)>(a)*(a)+(b)*(b)){
     numb.push([a,b])
      }
     }
   }
...
var px=Math.floor(getPlayerX()/16)
/*py=Math.floor(getPlayerY()/16)*/
var pz=Math.floor(getPlayerZ()/16);///
var pxyz//=""+(100000+px)+(100000+pz)


for(fx in numb){
a=numb[fx][0],b=numb[fx][1]
pxyz=""+(100000+px+a)+(100000+pz+b);
if(c[pxyz]==undefined){if(getTile(16*px,0,16*pz)>0){getchunk(c,pxyz,px,pz,cg,cb);break}}
pxyz=""+(100000+px+b)+(100000+pz+a);if(c[pxyz]==undefined){if(getTile(16*px,0,16*pz)>0){getchunk(c,pxyz,px,pz,cg,cb);break}}
a=-a
pxyz=""+(100000+px+a)+(100000+pz+b);
if(c[pxyz]==undefined){if(getTile(16*px,0,16*pz)>0){getchunk(c,pxyz,px,pz,cg,cb);break}}
pxyz=""+(100000+px+b)+(100000+pz+a);if(c[pxyz]==undefined){if(getTile(16*px,0,16*pz)>0){getchunk(c,pxyz,px,pz,cg,cb);break}}
...
a=numb[fx][0],b=numb[fx][1]
pxyz=""+(100000+px+a)+(100000+pz+b)
if(c[pxyz]!=undefined&&c[pxyz]!=null){
//print(c[pxyz].length)
for(var a=0;a<16;a++){
for(var b=0;b<16;b++){
var ps=c[pxyz]["s"+a+100+b]
if(ps!=undefined){
//px=0,pz=0
try{canvasTemp.drawRect((px*16+a)*kd,(py*16+b)*kd,(px*16+a)*kd+kd,(py*16+b)*kd+kd,ps);}catch(e){print(e)}
}else{clientMessage("s"+a+100+b)}
}}
}
}
...
newb=Matrix(newb,180-getYaw(),0.5*dp*ctx.getWindowManager().getDefaultDisplay().getHeight()*0.5*Math.SQRT2,0.5*dp*ctx.getWindowManager().getDefaultDisplay().getHeight()*0.5*Math.SQRT2)
...
var fd=1
var cr=new android.widget.CheckBox(ctx);
cr.setText("俯视模式(更新地图请重新开启小地图)");
cr.setOnCheckedChangeListener(new android.widget.CompoundButton.OnCheckedChangeListener()
			{
       onCheckedChanged:function(p1,p2)
				{
					if(p2) {fd=2} else {fd=1};
				}
		});
tmiLayout.addView(cr); 
tmiLayout.addView(add);
...
function sdqm(s1,s2,s3,s4,it,id)
{
if(s1!=false&&s2!=false)
{
var l=Math.max(vec.vlength(vec.vsubtract(s1,s2)),vec.vlength(vec.vsubtract(s4,s3)))
var x=s1[0],y=s1[1],z=s1[2],x2=s2[0],y2=s2[1],z2=s2[2]
var xx=s4[0],yy=s4[1],zz=s4[2],xx2=s3[0],yy2=s3[1],zz2=s3[2]

for(var m=0;m<=l;m+=0.25){
lianxian([Math.round(x+m/l*(x2-x)),Math.round(y+m/l*(y2-y)),Math.round(z+m/l*(z2-z))],[Math.round(xx+m/l*(xx2-xx)),Math.round(yy+m/l*(yy2-yy)),Math.round(zz+m/l*(zz2-zz))],it,id)}////
}}
...
var vec={
//长度(vector)
 vlength:function(a){return Math.sqrt(Math.pow(a[0],2)+Math.pow(a[1],2)+Math.pow(a[2],2))},
//a+b(vector,vector)
 vadding:function(a,b){return [a[0]+b[0],a[1]+b[1],a[2]+b[2]]},
//a-b(vector,vector)
 vsubtract:function(a,b){return [a[0]-b[0],a[1]-b[1],a[2]-b[2]]},
//零向量()
 vzero:function(){return [0,0,0]},
//a*b(vector,vector)
 vdot:function(a,b){return a[0]*b[0]+a[1]*b[1]+a[2]*b[2]},
//a x b(vector,vector)
 vcross:function(a,b){return [a[2]*b[3]-b[2]*a[3],a[3]*b[1]-b[3]*a[1],a[1]*b[2]-b[1]*a[2]]},
//两向量夹角的弧度(vector,vector)
 vangle:function(a,b){return Math.acos((a[0]*b[0]+a[1]*b[1]+a[2]*b[2])/(vec.length(a)*vec.length(b)))},
///向量单位向量化(vector)
 vunit:function(a){var l=vec.vlength(a);return [a[0]/l,a[1]/l,a[2]/l]},
///设置向量的模(负数反方向)(vector,number)
 vsetlength:function(a,m){var a=vec.vunit(a);return [a[0]*m,a[1]*m,a[2]*m]},
//向量放缩(负数反方向)(vector,number)
 vsetscaling:function(a,m){return [a[0]*m,a[1]*m,a[2]*m]},
//a绕着b旋转ang弧度(vector,vector,num_rad)
 vrotate:function(a,b,ang){
var x0=b[0],y0=b[1],z0=b[2]
var x2=b[0]*b[0],y2=b[1]*b[1],z2=b[2]*b[2]
var d2=x+y+z
var d=Math.sqrt(d2)
var sina=Math.sin(ang),b=Math.cos(ang)
return [(x2+(y2+z2)*cosa)/d2*a[0]+(x0*y0*(1-cosa)/d2 - z0*sina/d )* a[1]+(x0*z0*(1-cosa)/d2+y0*sina/d)*a[2],
(y0*x0*(1-cosa)/d2+z0*sina/d)*a[0]+ (y2+(x2+z2)*cosa)/d2*a[1]+ (y0*z0*(1-cosa)/d2-x0*sina/d)*a[2],
(z0*x0*(1-cosa)/d2 - y0*sina/d)*a[0]+ (z0*y0*(1-cosa)/d2+x0*sina/d)*a[1]+ (z2+(x2+y2)*cosa)/d2*a[2]]},
/////a朝着b旋转ang弧度(vector,vector,num_rad)
 vspin:function(a,b,ang){return vec.vrotate(a,vec.vcross(a,b),ang)},
////随机方向的单位向量()
 vramdom:function(){return vec.vunit([Math.ramdom(),Math.ramdom(),Math.ramdom()])}
}
}
